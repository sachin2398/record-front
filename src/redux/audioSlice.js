...
export const AddAudio = createAsyncThunk('audio/AddAudio', async (audio) => {
    try {
        const res = await axios.post('https://record-backend-vmc7.onrender.com/api/audio', audio)
        return res.data;
    } catch (error) {
        console.log(error)
    }
...
export const GetAudio = createAsyncThunk('audio/getAudios', async (audio) => {
    try {
        const response = await axios.get('https://record-backend-vmc7.onrender.com/api/audio', audio)
